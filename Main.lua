...
-- game cursors
local partCursor = workspace.Client.Cursor;
local uiCursor = client.PlayerGui.CursorGui;
local cursorHolder = workspace.Cursors;
local fakeCursor = Instance.new("CFrameValue");
local cursorColour;
...
cursorColour = miscSection:AddColor({
    text = "Cursor Colour";
    flag = "cursorColour";
    callback = function(colour)
        uiCursor.ImageLabel.ImageColor3 = colour;
        uiCursor.ImageLabel.BackgroundColor3 = colour;
    end;
});
miscSection:AddToggle({
    text = "Song Speed";
